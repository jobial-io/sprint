...
class Cache[F[_] : Concurrent, A, B](store: Ref[F, Map[A, CacheEntry[F, A, B]]], timeout: Duration,
  accessCount: Ref[F, Long], cleanupFrequency: Int, maximumSize: Option[Int]) extends CatsUtils[F] {

  def getOrCreate(key: A, value: F[B], onExpiry: (A, B) => F[Unit] = { (_: A, _: B) => unit }): F[B] =
    for {
